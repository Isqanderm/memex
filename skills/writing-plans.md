...
Before writing any plan, VERIFY that architectural artifacts exist:

- [ ] ADR decisions in `docs/architecture/adr/`
...
### Step 7: Save the Plan + Bridge to PR

```bash
mkdir -p .hermes/plans/
# Save plan to .hermes/plans/pr-XXX-description.md
git add .hermes/plans/
git commit -m "docs: add implementation plan for [feature]"
```

### Step 8: Transition to PR

After the plan is committed, the next step is ALWAYS to create a PR branch and follow the `github-pr-workflow`:

1. Branch off main: `git checkout -b pr-XXX-description`
2. Implement task-by-task following the plan
3. Push → open PR → wait for CI green → merge

**Rule:** Plans are not proposals to be discussed — they are the design phase. Commit them, then immediately branch and start implementing.
...
## Project-Level Scoping (GitHub Issues)

For **new projects** (not features within an existing codebase), prefer GitHub issues as the planning artifact instead of a markdown plan document. This gives the user ownership, visibility, and a familiar tracking mechanism.

## Terminal State: Bridge to Execution
