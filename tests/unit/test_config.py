...
def test_settings_openai_provider():
    s = Settings(**_base(llm_provider="openai", llm_model="gpt-4o", openai_llm_api_key="sk-llm"))
    assert s.llm_provider == "openai"
    assert s.llm_model == "gpt-4o"


def test_settings_claude_provider():
    s = Settings(**_base(llm_provider="claude", llm_model="claude-opus-4-5", anthropic_api_key="sk-ant-test"))
    assert s.llm_provider == "claude"


def test_settings_defaults_unchanged():
    s = Settings(**_base(llm_provider="openai", llm_model="gpt-4o-mini", openai_llm_api_key="sk-llm"))
...
def test_missing_llm_provider_raises():
    with pytest.raises(ValidationError, match="llm_provider"):
        Settings(**_base(llm_model="gpt-4o"))


def test_missing_llm_model_raises():
    with pytest.raises(ValidationError, match="llm_model"):
        Settings(**_base(llm_provider="openai"))


def test_missing_database_url_raises():
    with pytest.raises(ValidationError):
        Settings(openai_api_key="sk-test", llm_provider="openai", llm_model="gpt-4o", openai_llm_api_key="sk-llm")
...
def test_claude_without_anthropic_key_raises():
    with pytest.raises(ValidationError, match="ANTHROPIC_API_KEY"):
        Settings(**_base(llm_provider="claude", llm_model="claude-opus-4-5"))


def test_claude_without_anthropic_key_raises_even_with_openai_llm_key():
    with pytest.raises(ValidationError, match="ANTHROPIC_API_KEY"):
        Settings(**_base(llm_provider="claude", llm_model="claude-opus-4-5", openai_llm_api_key="sk-llm"))


def test_openai_without_llm_api_key_raises():
    with pytest.raises(ValidationError, match="OPENAI_LLM_API_KEY"):
        Settings(**_base(llm_provider="openai", llm_model="gpt-4o"))


def test_openai_without_llm_api_key_raises_even_with_anthropic_key():
    with pytest.raises(ValidationError, match="OPENAI_LLM_API_KEY"):
        Settings(**_base(llm_provider="openai", llm_model="gpt-4o", anthropic_api_key="sk-ant-test"))


def test_invalid_provider_raises():
    with pytest.raises(ValidationError):
        Settings(**_base(llm_provider="gemini", llm_model="gemini-pro", openai_llm_api_key="sk-llm"))
