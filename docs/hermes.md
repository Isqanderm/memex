...
Skip to [Verify](#verify) after it completes.
...
Copy the network name exactly — you will need it in Step 3.
...
If that network does not exist, Compose will error — check `docker network ls` again.
...
connection takes time. Do not test in an existing chat session; open a new one or use the API
...
| Error | Cause | Fix |
|---|---|---|
| `Permission denied: '/opt/data/...'` | Hermes `chown` still running at startup | Use `/opt/hermes/.venv/bin/python3` — it is outside the volume |
